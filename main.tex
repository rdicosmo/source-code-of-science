% The Source Code of Science — a doctoral course note.
% Build with:  latexmk -xelatex main.tex   (or: make)
\documentclass[11pt,oneside]{memoir}

% Release version: DERIVED, not declared. .latexmkrc reads it from the annotated
% tag (`git describe`) and writes it into the git-ignored gitinfo.tex, so the tag
% is the single source of truth and there is nothing to keep in sync by hand.
% CONSEQUENCE FOR THE RELEASE FLOW: tag before building. Tagging does not change
% the tree, so the source SWHID is unaffected, but the version on the title page
% is only correct once the tag exists.  See appendix/J-self-archival.tex.

\input{preamble}

\begin{document}

\swhtitlepage

% --- Abstract / how to read these notes ----------------------------------
\setchapvignette[6.4cm]{about-pedestals}
\chapter*{About these notes}
\addcontentsline{toc}{chapter}{About these notes}
These notes are a doctoral-level course on managing the \emph{source code} of research:
how to archive it for the long term, reference it at the right granularity, describe and
cite it, and use it to make computational results reproducible. They are built on Software
Heritage\footnote{In the interest of transparency: the author helped found Software Heritage,
co-developed the \texttt{biblatex-software} style used here, and contributed to the \textsc{swhid}
standard; the recurring worked example (the Parmap library) is also his. The argument is meant to
stand on its evidence, and the sources cited throughout are independent.} and the \textsc{swhid} ---
an identifier computed from the code itself, defined in Chapter~\ref{ch:identifiers} and now the
international standard ISO/IEC~18670:2025. They address
the FAIR principles and then go \emph{beyond} them, following the CODE-beyond-FAIR roadmap for
research software~\autocite{NatureSD2026}. The acronyms in this paragraph are all explained, in
order, in the chapters that follow; nothing here needs to be known in advance.

These notes are meant to be read \emph{and} done: each practical chapter ends with a
\textbf{Hands-on} box. Margin notes flag the key takeaways.

\begin{callout}{In one page (for the busy reader)}
\small
\textbf{The claim.} Source code is a first-class research output, but the habit of depositing a
\texttt{.zip} with a DOI fails it: a DOI certifies a landing page, not the bytes; the deposit can
change, has no granularity, and discards history. The fix is to archive the living repository in
\textbf{Software Heritage} and reference it with a \textbf{\textsc{swhid}} --- an identifier computed
from the content itself, recomputable and verifiable offline, and the international standard
ISO/IEC~18670.

\textbf{The evidence.} The archive holds the development history of more than 29~billion source
files from over 440~million projects. The \textsc{swhid} is already accepted by ACM for the
\emph{Artifacts Available} badge; \emph{eLife} has auto-archived every article's code since 2020;
HAL, IPOL, swMATH, and Episciences are wired in; UNESCO, EOSC, France's National Plan, and the ANR
all point research software at Software Heritage.

\textbf{What to do.}
\emph{Researchers}: archive your code, cite it with a \textsc{swhid} (Chapter~\ref{ch:ardc}).
\emph{Editors / artifact-evaluation committee (AEC) chairs}: require a \textsc{swhid} for the \emph{Available} badge --- a reviewer
can verify it offline (Chapter~\ref{ch:repro}, with a ready call-for-artifacts box).
\emph{Funders / policy makers}: name Software Heritage and the \textsc{swhid} in conditions and
policy (Chapter~\ref{ch:outlook}, with a model clause and a governance/sustainability note).
\end{callout}

\tableofcontents

% --- Chapters ------------------------------------------------------------
\include{chapters/01-why}
\include{chapters/02-beyond-fair}
\include{chapters/03-foundations}
\include{chapters/04-identifiers}
\include{chapters/05-ardc}
\include{chapters/06-reproducibility-aec}
\include{chapters/07-outlook}

% --- Appendices ----------------------------------------------------------
\appendix
\include{appendix/A-cookbook}
\include{appendix/B-swhid-syntax}
\include{appendix/C-glossary}
\include{appendix/D-swhid-tools}
\include{appendix/E-policy}
\include{appendix/F-venue-classes}
\include{appendix/G-aec-model-section}
\include{appendix/H-archie}
\include{appendix/I-citing}
\include{appendix/J-self-archival}

% --- References ----------------------------------------------------------
\printbibliography[heading=refschapter,title={References}]

% --- Colophon (self-archival SWHID + capstone) parked in annex-colophon.tex;
%     not included for now. See that file's header for why.

\end{document}
