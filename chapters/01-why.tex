...
its full development history, and the \textsc{swhid}, an identifier computed \emph{from the code
itself} (and now the international standard ISO/IEC~18670) that lets you point at exactly the code
you mean. \textsc{ardc} is the spine of the whole note; Chapter~\ref{ch:ardc} makes each of its
four verbs hands-on. We will meet the FAIR principles along the way and then, deliberately, go
\emph{beyond} them, for the reasons the next chapter makes precise~\autocite{NatureSD2026}.
...
forge, qualified identifier --- is defined in the Glossary (Appendix~\ref{app:glossary}). And the
note practises what it preaches: it is itself archived in Software Heritage, and the Colophon shows
you how to recompute and verify its own identifier --- the entire workflow, applied to the very
...
\begin{description}[nosep,leftmargin=2.6cm,style=nextline,font=\bfseries\sffamily]
  \item[Researcher] read straight through; Chapters~\ref{ch:ardc} (the practice) and
        \ref{ch:repro} (reproducibility and artifact evaluation) are where the work gets done.
  \item[Developer / intern] if you build software but are not (yet) writing papers, go to
        Chapter~\ref{ch:ardc}, \S\,Archive, plus the licence advice and
        Appendix~\ref{app:cookbook}; the citation, artifact-evaluation, and policy chapters are
        optional.
  \item[Already convinced] skip to Chapter~\ref{ch:identifiers} (what a \textsc{swhid} is) and
        Chapter~\ref{ch:ardc} (how to use it); the foundations are there if you want them.
  \item[Editor / funder / policy maker] Chapters~\ref{ch:repro}--\ref{ch:outlook} and the
        ready-to-use boxes therein are addressed to you; you need not run the labs.
\end{description}
\end{callout}

If you write or run code for your research --- whether a few lines of analysis or a library
maintained for decades --- this note is for you. The tools are ready; the habit is the only thing
