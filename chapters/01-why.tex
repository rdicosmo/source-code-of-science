\setchapvignette{ch1-paper-kept-code-lost}
\chapter{Why these notes}
\label{ch:why}

\chapsummary{Source code is a research output in its own right, and the one we most often lose: the article
is archived, the data deposited, and the code left on a forge that promises nothing.
Reproducibility and preservation are only the most visible stakes; from the lone result to the
institutional dashboard, every need rests on the same floor --- archive, reference, describe,
cite.}
...
\begin{figure}[htbp]
  ...
  orange the real system libraries beneath, green the artifacts of packaging
...
repositories at risk~\autocite{google-code-shutdown,gitorious-shutdown}; Bitbucket deleted more
than 250,000 Mercurial repositories in 2020~\autocite{bitbucket-mercurial-sunset}. The
half-life of a URL cited in the scholarly literature is about four years~\autocite{spinellis2003decay}. Every one of those
...
and you can adopt them today. It rests on two foundations --- the Software Heritage archive~\autocite{softwareheritage}, which
...
opens with a short summary and its \textbf{learning objectives}; and any unfamiliar term --- \textsc{swhid}, snapshot,
forge, qualified identifier --- is defined in the Glossary (Appendix~\ref{app:glossary}).
...
\textbf{6}~Reproducibility \enspace \textbf{7}~Policy. \quad App.: \textbf{A}~Cookbook \enspace \textbf{E}~Policy reference.}
\end{callout}
