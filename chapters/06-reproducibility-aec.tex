...
Everything so far has been preparation. \textsc{ardc} gives us a way to keep code and to name it
exactly; this chapter is the test of why that matters. The sharpest test our community has is
\emph{artifact evaluation} --- the practice, now standard at computer-science conferences, of
checking that the software behind a paper actually exists, runs, and supports its claims --- and it
turns out that Software Heritage answers its central need almost perfectly.
...
this densely: \emph{Source Code Hotspots} locates exact passages with thirty
\textsc{swhid}s~\autocite{spinellis2026hotspots}. In a qualified \textsc{swhid} the \emph{identifying}
part is precisely the qualifiers --- \texttt{origin}, \texttt{visit}, \texttt{anchor} --- while the
core hash reveals nothing about authorship. So the \emph{only} difference between the double-blind
